...
describe("shouldRestoreTerminalFocus", () => {
  ...
  it("restores when nothing holds focus (OS app-switch drops focus onto <body>)", () => {
    expect(shouldRestoreTerminalFocus(null, host, body)).toBe(true);
    expect(shouldRestoreTerminalFocus(body, host, body)).toBe(true);
  });

  it("restores when the terminal itself already had focus", () => {
    expect(shouldRestoreTerminalFocus(textarea, host, body)).toBe(true);
  });

  it("does not steal focus from another control on the page", () => {
    expect(shouldRestoreTerminalFocus(sidebarInput, host, body)).toBe(false);
  });

  it("restores when the terminal host is not mounted yet", () => {
    expect(shouldRestoreTerminalFocus(sidebarInput, null, body)).toBe(true);
  });
});
